...
import de.llalon.gitbackup.model.GitlabRepo;
...
@Slf4j
@AllArgsConstructor
public class GitLabHandler implements GitHandler {
    private static final String ENDPOINT = "/api/v4/projects?membership=true";
    ...
    private Map.Entry<String, String> getAuthHeader() {
        ...
        return Map.entry("PRIVATE-TOKEN", config.getToken());
    }
    ...
    private List<GitRepo> getRepoPage(int page) throws URISyntaxException, IOException, InterruptedException {
        log.debug("Getting repos page {} from Gitlab", page);
        ...
        CollectionType typeReference =
                TypeFactory.defaultInstance().constructCollectionType(List.class, GitlabRepo.class);

        if (response.statusCode() != 200) {
            log.error("Gitlab API returned with non 200 status code: {}", response.statusCode());
